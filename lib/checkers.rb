...
class Piece
  ...
  def initialize(color, board, pos)
    ...
    raise "invalid pos" unless board.pos_within_bounds?(pos)
  ...
  def valid_moves
    ...
    move_diffs.each do |(dx, dy)|
      ...
      #p pos
      #next unless board.pos_within_bounds?(pos)
      if board.pos_within_bounds?(pos)
        ...
        #its jump move diff must be greater than 1, see move_diffs for clarity
          ...
          p "pos: #{pos}"
          if board[pos].color != self.color && dx.abs > 1
            valid_moves << pos
          end
        end
      end
  ...
  def perform_moves!(move_sequence, a_board = self.board)
    ...
    if move_sequence.count == 1
      #begin
      p "self.pos #{self.pos} and move_sequence.first #{move_sequence.first}"
      p !jump?(self.pos, move_sequence.first)
      p "valid moves #{self.valid_moves}"
      if (self.pos[0] - move_sequence.first[0]).abs != 2 && self.valid_moves.include?(move_sequence.first)
        "Slide is performing"
        perform_slide(move_sequence.first)
        #rescue 
      elsif self.valid_moves.include?(move_sequence.first)#may not be legit
        p "Jump is performing"
  ...
  def jump?(start_pos, end_pos)
    p "jump #{(start_pos[0] - end_pos[0]).abs == 2}"
  ...
  def move_diffs
    ...
    if self.color == :black
      ...
      move_diffs.map! { |move_diff| move_diff.map! { |diff| diff * -1 } }
      p ":black #{move_diffs}"
    end
    move_diffs  
  end
...
class Board
  ...
  #protected
  def []=(pos, piece)
    raise "invalid pos" unless pos_within_bounds?(pos)
    ...
    (0...grid.length).each do |row|
      ...
      (0...grid[row].length).each do |col|
        ...
        if square.is_a?(Piece)
          ...
          ################################
          print space_render(square_color) + ' '
          #######################
        end
        square_color = opposing_color(square_color)
      end
...
class HumanPlayer
  ...
  def get_pos(prompt, board)
    puts prompt
    move_hash[gets.chomp]
  end
  
  def get_sequence(prompt, board)
    puts prompt
    gets.chomp.split(" ").map { |move| move_hash[move]}
  end
end
